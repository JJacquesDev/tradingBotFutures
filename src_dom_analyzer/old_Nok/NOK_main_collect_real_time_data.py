...
from classes.real_time_data_signalr import Real_time_data
...
def collect_real_time_data(token):

    # JWT_TOKEN = "your_bearer_token"
    MARKETHUB_URL = "https://rtc.topstepx.com/hubs/market"
    CONTRACT_ID = "CON.F.US.RTY.H25"
    try:
        o_real_time_data = Real_time_data(token, MARKETHUB_URL, CONTRACT_ID)
    except Exception as e:
        print("ERROR IN COMMUNICATION WITH MARKET HUB! : ", e)
...
if __name__=="__main__":
    ...
    token = log_to_api_projectx()
    print(token)
    # pick_up_accounts_data()
    # collect_real_time_data(token)